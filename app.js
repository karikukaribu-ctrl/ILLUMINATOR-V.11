...
function bootstrapDemo() {
  ...
  projects.forEach((p, i) => {
    p.x = positions[i]?.x || 120 + i * 120
    p.y = positions[i]?.y || 100 + i * 80
  })
...
function bindGate() {
  qs("#doorContinue").addEventListener("click", () => {
    enterMiddleEarth("Carte centrale")
  })

  qs("#doorStart").addEventListener("click", () => {
    enterMiddleEarth("Premier pas")
    const frag = Engine.suggestFragment()
    if (frag) setTimeout(() => openFragmentQuickModal(frag.projectId, frag.id, true, true), 120)
  })

  qs("#doorExplore").addEventListener("click", () => {
    enterMiddleEarth("Explorer les territoires")
  })

  qs("#doorDump").addEventListener("click", () => {
    enterMiddleEarth("Décharger d’abord")
    setTimeout(() => openNotesModal(), 120)
  })

  qs("#doorCenter").addEventListener("click", () => {
    enterMiddleEarth("Réduction du bruit")
    Engine.state.preferences.focus = false
    Engine.state.preferences.fog = "light"
    Engine.save()
    renderAll()
  })
}

function enterMiddleEarth(hint = "") {
  qs("#middleEarthGate").classList.remove("is-show")
  qs("#floatingDock").classList.remove("hidden")
  qs("#appShell").classList.remove("hidden")
  qs("#gateHint").textContent = hint || ""
...
function bindActions() {
  ...
  qs("#forgeBtn").addEventListener("click", openForgeModal)
  qs("#portalBtn").addEventListener("click", openPortalModal)
...
function bindPrefs() {
  qs("#savePrefsBtn").addEventListener("click", () => {
    ...
    Engine.state.preferences.fog = qs("#fogModeSelect").value
    ...
    Engine.state.preferences.weather = qs("#weatherSelect").value
...
function syncPrefsUI() {
  ...
  qs("#fogModeSelect").value = Engine.state.preferences.fog || "off"
  ...
  qs("#weatherSelect").value = Engine.state.preferences.weather || "clear"
  qs("#globalNotesArea").value = Engine.state.globalNotes || ""
  applyPrefsToBody()
}

function applyPrefsToBody() {
  document.body.classList.remove("theme-printemps", "theme-ete", "theme-automne", "theme-hiver", "mode-dark", "fog-light", "fog-strong")
  document.body.classList.add(`theme-${Engine.state.preferences.season}`)
  if (Engine.state.preferences.mode === "dark") document.body.classList.add("mode-dark")
  if (Engine.state.preferences.fog === "light") document.body.classList.add("fog-light")
  if (Engine.state.preferences.fog === "strong") document.body.classList.add("fog-strong")
}
...
function renderStatus() {
  ...
  const weatherLabels = {
    clear: "ciel clair",
    fog: "brume",
    wind: "vent de dispersion",
    storm: "orage"
  }
  qs("#weatherLabel").textContent = `Météo : ${weatherLabels[Engine.state.preferences.weather] || "ciel clair"}`
}
...
function renderFragmentList() {
  const root = qs("#leftFragmentList")
  root.innerHTML = ""
  ...
  fragments.forEach(fragment => {
    const item = el("div", "active-fragment-item")
    item.innerHTML = `
...
function renderMuseum() {
  const root = qs("#museumList")
  root.innerHTML = ""

  if (!Engine.state.museum.length) {
    root.innerHTML = `<div class="small-muted">Aucun projet en sommeil.</div>`
    return
  }

  Engine.state.museum.forEach(project => {
    const row = el("div", "project-list-item")
    row.innerHTML = `
      <strong>${escapeHTML(project.name)}</strong>
      <div class="small-muted">En sommeil</div>
    `
    row.addEventListener("click", () => {
      Engine.restoreProject(project.id)
      renderAll()
    })
...
function renderMapView() {
  ...
  projects.forEach(project => {
    const node = el("div", "realm-node", `${escapeHTML(project.name)}<br>${project.progress}%`)
    ...
    if (project.expanded) {
      project.fragments.forEach((fragment, idx) => {
        ...
        const fragNode = el(
          "div",
          `fragment-node ${fragment.done ? "is-done" : "is-pending"}`,
          escapeHTML(fragment.title.length > 18 ? fragment.title.slice(0, 18) + "…" : fragment.title)
        )
        ...
        fragNode.draggable = true
        fragNode.addEventListener("dragstart", e => {
  ...
  const planningZones = [
    { id: "zone-today", label: "Aujourd’hui", x: 40, y: 20, date: App.selectedDate },
    { id: "zone-tomorrow", label: "Demain", x: 40, y: 90, date: addDays(App.selectedDate, 1) },
    { id: "zone-week", label: "Cette semaine", x: 40, y: 160, date: startOfWeekISO(App.selectedDate) }
  ]

  planningZones.forEach(zone => {
    const z = el("div", "project-list-item", `<strong>${zone.label}</strong>`)
    z.style.position = "absolute"
    z.style.left = `${zone.x}px`
    z.style.top = `${zone.y}px`
    z.style.width = "130px"
    ...
    z.addEventListener("drop", e => {
      ...
      try {
        ...
        if (data.type === "fragment") {
          Scheduler.moveFragmentToDate(data.projectId, data.fragmentId, zone.date)
          renderAll()
        }
      } catch {}
    })
...
function attachProjectNodeDrag(node, project) {
  ...
  node.addEventListener("pointermove", e => {
    if (!App.mapDrag || App.mapDrag.id !== project.id) return
...
function renderDayView() {
  ...
  const buckets = [
    { label: "08h — 09h", from: 8 * 60, to: 9 * 60, date: App.selectedDate },
    { label: "Travail journée", from: 9 * 60, to: 18 * 60, date: App.selectedDate },
    { label: "20h — 21h30", from: 20 * 60, to: 21 * 60 + 30, date: App.selectedDate },
    { label: "21h30 — 23h", from: 21 * 60 + 30, to: 23 * 60, date: App.selectedDate }
  ]
  ...
  buckets.forEach(bucket => {
    ...
    cell.addEventListener("drop", e => {
      ...
      try {
        const data = JSON.parse(e.dataTransfer.getData("text/plain"))
        if (data.type === "fragment") {
          Scheduler.moveFragmentToDate(data.projectId, data.fragmentId, bucket.date)
          renderAll()
        }
      } catch {}
    })
  ...
  for (let i = 0; i < 7; i++) {
    ...
    day.addEventListener("drop", e => {
      ...
      try {
        const data = JSON.parse(e.dataTransfer.getData("text/plain"))
        if (data.type === "fragment") {
          Scheduler.moveFragmentToDate(data.projectId, data.fragmentId, iso)
          renderAll()
        }
      } catch {}
    })
...
function openProjectDetailModal(projectId) {
  ...
  openModal(`
    ...
    <hr>
    <div id="projectFragmentsModal">
      ${project.fragments.map(f => `
        <div class="project-fragment-row" data-fragment-id="${f.id}">
    ...
    <button id="archiveProjectBtn" class="action-btn">Musée</button>
  `, () => {
    qsa("#projectFragmentsModal .project-fragment-row").forEach(row => {
    ...
    qs("#archiveProjectBtn").addEventListener("click", () => {
...
function openFragmentDetailModal(projectId, fragmentId) {
  ...
  openModal(`
    ...
    <button id="forgeFragBtn" class="action-btn">Forgeron</button>
  `, () => {
    ...
    qs("#forgeFragBtn").addEventListener("click", () => {
      Engine.splitFragment(projectId, fragmentId)
      closeModal()
      renderAll()
    })
  })
...
function openProjectChooserModal() {
  openModal(`
    ...
    <div id="projectChooserList" class="stack">
      ${Engine.state.projects.map(p => `<div class="project-fragment-row" data-project-id="${p.id}">${escapeHTML(p.name)} · ${p.progress}%</div>`).join("")}
    </div>
  `, () => {
    qsa("#projectChooserList .project-fragment-row").forEach(row => {
      row.addEventListener("click", () => {
        closeModal()
...
function openForgeModal() {
  ...
  openModal(`
    <h2>Forgeron</h2>
    <p>Découpe un fragment trop massif en étapes plus respirables.</p>
    ${best ? `<p><strong>Suggestion :</strong> ${escapeHTML(best.title)}</p>` : `<p>Aucun fragment suggéré.</p>`}
    <button id="forgeSuggestedBtn" class="action-btn action-btn--accent">Forger le fragment suggéré</button>
  `, () => {
    qs("#forgeSuggestedBtn").addEventListener("click", () => {
      if (!best) return
      Engine.splitFragment(best.projectId, best.id)
      closeModal()
      renderAll()
    })
  })
...
function openPortalModal() {
  openModal(`
    <h2>Portail</h2>
    <p>Capture une pensée parasite et transforme-la en fragment.</p>
    <label>Texte<br><textarea id="portalText" rows="6" class="field textarea"></textarea></label><br><br>
    <button id="portalToInboxBtn" class="action-btn action-btn--accent">Vers le premier projet</button>
  `, () => {
    qs("#portalToInboxBtn").addEventListener("click", () => {
      const text = qs("#portalText").value.trim()
      if (!text) return
