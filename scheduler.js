...
Scheduler.moveFragmentToDate = (projectId, fragmentId, isoDate) => {
  const fragment = Engine.getFragment(projectId, fragmentId)
  if (!fragment) return null

  const existing = Engine.state.scheduledBlocks.find(b => b.projectId === projectId && b.fragmentId === fragmentId)
  if (existing) {
    Scheduler.deleteBlock(existing.id)
  }

  return Scheduler.scheduleFragment(projectId, fragmentId, Scheduler.fromISO(isoDate))
}
...
Scheduler.getTimelineRows = isoDate => {
  ...
  const blocks = Scheduler.getBlocksForDate(isoDate).map(block => {
    ...
    return { ...block, title, projectName, color }
  })
