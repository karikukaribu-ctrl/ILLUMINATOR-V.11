...
Engine.archiveProject = function(projectId) {
  const project = Engine.state.projects.find(p => p.id === projectId)
  if (!project) return
  Engine.state.projects = Engine.state.projects.filter(p => p.id !== projectId)
  Engine.state.museum.push(project)
  Engine.save()
}

Engine.restoreProject = function(projectId) {
  const project = Engine.state.museum.find(p => p.id === projectId)
  if (!project) return
  Engine.state.museum = Engine.state.museum.filter(p => p.id !== projectId)
  Engine.state.projects.push(project)
  Engine.save()
}
...
Engine.splitFragment = function(projectId, fragmentId) {
  const project = Engine.getProject(projectId)
  const fragment = Engine.getFragment(projectId, fragmentId)
  ...
  const idx = project.fragments.findIndex(f => f.id === fragmentId)
  ...
  const fragB = {
    ...
    duration: fragment.duration - half,
    done: false,
    scheduled: null
  }

  project.fragments.splice(idx, 1, fragA, fragB)
  Engine.updateProjectProgress(projectId)
  Engine.save()
}
